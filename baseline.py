...
def unweighted_average(DATA_PATH="data/de_train.parquet", ID_PATH="data/id_map.csv"):
    ...
    for _, row in testing_pairs.iterrows():
        ...
        average = (
            data.loc[data["sm_name"] == compound].to_numpy()[:, 5:18216].mean(axis=0)
        )
        results.append(average)
    results = pd.DataFrame(results)
    SUBMISSION_PATH = os.path.join(
        os.path.dirname(__file__), "submissions/avg_baseline_submission.csv"
    )
...
def leave_one_out(
    DATA_PATH="data/de_train.parquet",
    model=VectorWeights,
    epochs=10000,
    lr=1e-2,
    ...
    data = pd.read_parquet(
        "/Users/iishaan/all-code/kaggle/single-cell/data/de_train.parquet",
        engine="fastparquet",
    )
    ...
    for compound in compounds:
        ...
        m_cell = (
            rows.loc[rows["cell_type"] == "Myeloid cells"].iloc[0, 5:18216].to_numpy()
        )
        t_c4 = rows.loc[rows["cell_type"] == "T cells CD4+"].iloc[0, 5:18216].to_numpy()
        t_r = (
            rows.loc[rows["cell_type"] == "T regulatory cells"]
            .iloc[0, 5:18216]
            .to_numpy()
        )
...
def make_predictions(
    ...
    for i, pair in pairs.iterrows():
        ...
        t_c4 = (
            cells.loc[cells["cell_type"] == "T cells CD4+"]
            .iloc[0, 5:18216]
            .to_numpy(dtype="float32")
        )
        t_r = (
            cells.loc[cells["cell_type"] == "T regulatory cells"]
            .iloc[0, 5:18216]
            .to_numpy(dtype="float32")
        )
        nk = (
            cells.loc[cells["cell_type"] == "NK cells"]
            .iloc[0, 5:18216]
            .to_numpy(dtype="float32")
        )
...
if __name__ == "__main__":
    # Average Model: 0.627
    m_model, b_model = train_2_models(
        model=WeightedSum, epochs=1500, lr=6e-2, device=torch.device("mps")
    )
    make_predictions(
        m_model, b_model, FILE_PATH="submissions/MAE_w_sum_baseline_submission.csv"
    )